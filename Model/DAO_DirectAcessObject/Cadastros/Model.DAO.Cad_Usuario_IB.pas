...
Type
   TDAO_Cad_Usuario_IB = Class(TInterfacedObject, IDAO_Cad_Usuario)
   Public
      Query: TIBQuery;
      DataSet: TDataSource;
...
class function TDAO_Cad_Usuario_IB.New: IDAO_Cad_Usuario;
begin
   Self.Create;
end;
...
constructor TDAO_Cad_Usuario_IB.Create;
begin
   IB_Componente := TConexao_IB.Create;
   Query:= TIBQuery.Create(Nil);
//   Query.Database := TIBDatabase(IB_Componente.Connection);
   Query.Database := IB_Componente.FDatabase;
...
destructor TDAO_Cad_Usuario_IB.Destroy;
begin
  inherited;
...
function TDAO_Cad_Usuario_IB.GetSimple(vsLixeira: String): Boolean;
begin
   if vsLixeira ='' then
      vsLixeira := 'NAO';
   with Query do
...
function TDAO_Cad_Usuario_IB.GetId(vsId, vsLixeira: String): Boolean;
begin
   if vsLixeira ='' then
      vsLixeira := 'NAO';
   with Query do
   ...
   try
      ...
      begin
         ShowMessage('[MODEL-GetID] Erro ao acessar dados: ' +#13+#13+ nomeErro.Message);
         Result := False;
         Exit;
      end;
   end;
...
function TDAO_Cad_Usuario_IB.Get(vsCampos, vsCondicao, vsOrdem, vsLixeira: String): Boolean;
var
   vsQuery: String;
   vsIniQuery: String;
begin
   ...
   begin
      ...
      vsQuery := SQL.Text;
      SQL.SaveToFile('C:\DF_LocacaoM1\Parametros\SqlCadUsuarioQuery.Txt');
   end;
   try
      Query.Prepare;
      Query.Open;
...
function TDAO_Cad_Usuario_IB.Post (ORM: TORM_LOC_M1_CAD_USUARIO): Boolean;
VAR
   vsErro1 : String;
begin
   ...
   Try
      ...
      begin
         ...
         SQL.Add(' ''NAO'', ');
         SQL.Add('-1)');
         SQL.SaveToFile('C:\DF_LocacaoM1\Parametros\SqlCadUsuarioStoreProcedure.Txt');
         ExecSQL;
         SQL.Clear;
         SQL.Add('COMMIT');
         ExecSQL;
...
function TDAO_Cad_Usuario_IB.Put (ORM: TORM_LOC_M1_CAD_USUARIO): Boolean;
VAR
   vsErro1 : String;
begin
   if Query.Transaction.Active = False then
...
function TDAO_Cad_Usuario_IB.Delete(ORM: TORM_LOC_M1_CAD_USUARIO): Boolean;
VAR
   vsErro1 : String;
begin
   if Query.Transaction.Active = False then
...
function TDAO_Cad_Usuario_IB.CriarTabela: string;
var
